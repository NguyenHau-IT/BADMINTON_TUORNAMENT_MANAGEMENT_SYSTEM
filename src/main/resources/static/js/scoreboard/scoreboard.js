...
$(document).ajaxError(function(event, xhr, settings) {
  // Handle verification errors globally
  if (xhr.status === 403 && settings.url.includes('/api/court/')) {
    try {
      const errorData = JSON.parse(xhr.responseText);
      if (errorData.error === 'Chưa được duyệt') {
        // Show waiting approval modal
        showWaitingApprovalModal(errorData.message);
        return;
      } else if (errorData.error === 'Thiết bị bị chặn') {
        // Show blocked modal
        showBlockedModal(errorData.message);
        return;
      }
    } catch (e) {
      // Not JSON, let individual handlers deal with it
    }
  }
});

function showWaitingApprovalModal(message) {
  const modalHtml = `
    <div class="modal fade" id="waitingApprovalModalScoreboard" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-warning">
          ...
          <div class="modal-body text-center py-4">
            ...
            <p class="mt-3 mb-0 fs-5">${message || 'Đang chờ quản trị viên duyệt...'}</p>
          ...
          <div class="modal-footer justify-content-center">
            ...
            <button type="button" class="btn btn-primary" onclick="window.location.href='/pin'">Quay lại trang PIN</button>
          </div>
        </div>
      </div>
    </div>
  ...
  $('#waitingApprovalModalScoreboard').remove();
  $('body').append(modalHtml);
  const modal = new bootstrap.Modal(document.getElementById('waitingApprovalModalScoreboard'));
  modal.show();
...
function showBlockedModal(message) {
  const modalHtml = `
    <div class="modal fade" id="blockedModalScoreboard" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-danger">
          <div class="modal-header bg-danger text-white">
            <h5 class="modal-title"><i class="bi bi-x-circle-fill me-2"></i>Thiết bị bị chặn</h5>
          </div>
          <div class="modal-body text-center py-4">
            <i class="bi bi-shield-x text-danger" style="font-size: 4rem;"></i>
            <p class="mt-3 mb-0 fs-5">${message || 'Thiết bị của bạn đã bị chặn'}</p>
  ...
  $('#blockedModalScoreboard').remove();
  $('body').append(modalHtml);
  const modal = new bootstrap.Modal(document.getElementById('blockedModalScoreboard'));
  modal.show();
}
    ...
    .fail((xhr) => {
      ...
      if (xhr.status === 403) {
        try {
          const errorData = JSON.parse(xhr.responseText);
          if (errorData.error === 'Chưa được duyệt') {
            showToast('toastError', '⏳ ' + errorData.message + '\n\nVui lòng chờ quản trị viên duyệt thiết bị của bạn.');
            return;
          }
        } catch (e) {
...
function checkGameWinCondition(source) {
  ...
  if (matchEnded) {
    ...
    hideNextGameSuggestion();
    // Show "View Result" button when match ends
    $('#btnViewResult').removeClass('d-none');
    return;
  } else {
    // Hide "View Result" button when match is ongoing
    $('#btnViewResult').addClass('d-none');
  }
...
function showMatchEndedMessage(gamesA, gamesB) {
  // Immediately redirect to result page without countdown
  const pin = getPinCodeFromUrl();
  if (pin) {
    window.location.href = '/result/' + pin;
  }
}
