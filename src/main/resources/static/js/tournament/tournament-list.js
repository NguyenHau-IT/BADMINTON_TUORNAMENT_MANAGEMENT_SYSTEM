...
// ========== LOGGING HELPER ==========
// Use BTMSUtils.log if available, fallback to no-op for production
const log = (window.BTMSUtils && window.BTMSUtils.log) || {
    debug: () => {},
    info: () => {},
...
window.openQuickView = async function(tournamentId) {
    log.debug('Opening quick view for tournament:', tournamentId);
    
    // Get modal elements
    const modalEl = document.getElementById('quickViewModal');
    if (!modalEl) {
        log.error('Quick View Modal element not found!');
        return;
    }
    ...
    try {
        ...
        log.debug('Tournament data loaded:', tournament);
    ...
    cards.forEach(card => {
        card.addEventListener('click', function(e) {
            if (!e.target.closest('.btn')) {
                ...
                log.debug('Tournament card clicked:', tournamentId);
...
function initViewToggle() {
    log.debug('🎯 Initializing view toggle...');
    
    const toggleButtons = document.querySelectorAll('.btn-view-toggle');
    const container = document.getElementById('tournamentsContainer');
    
    log.debug('Toggle buttons found:', toggleButtons.length);
    log.debug('Container found:', container ? 'Yes' : 'No');
    
    if (!toggleButtons.length || !container) {
        console.error('❌ View toggle elements not found!');
        log.debug('- Buttons:', toggleButtons.length);
    ...
    log.debug('Saved view mode:', savedView);
    setViewMode(savedView);

    // Add click handlers
    toggleButtons.forEach((btn, index) => {
        log.debug(`Adding click handler to button ${index}:`, btn.dataset.view);
        
        btn.addEventListener('click', function(e) {
            // Don't prevent default for buttons
            const viewMode = this.dataset.view;
            log.debug('View toggle clicked:', viewMode);
    ...
    function setViewMode(mode) {
        log.debug('📐 Setting view mode to:', mode);
        
        // Update buttons
        toggleButtons.forEach(btn => {
            if (btn.dataset.view === mode) {
                btn.classList.add('active');
                log.debug('✅ Activated button:', mode);
            } else {
        ...
        if (mode === 'list') {
            ...
            log.debug('📋 Switched to list view');
        } else {
            container.classList.remove('list-view');
            container.classList.add('grid-view');
            log.debug('📱 Switched to grid view');
        }
        
        log.debug('Container classes:', container.className);
    }

    log.debug('✅ View toggle initialized successfully');
}
...
function initAdvancedFilters() {
    ...
    if (!filterForm) {
        log.debug('Advanced filter form not found');
        return;
    }
    ...
    if (filterBody && toggleBtn && toggleIcon && toggleText) {
        filterBody.addEventListener('shown.bs.collapse', function() {
            ...
            log.debug('Filter panel expanded');
        });
        ...
        filterBody.addEventListener('hidden.bs.collapse', function() {
            ...
            log.debug('Filter panel collapsed');
        });
    ...
    // Apply Filters - AJAX mode (không reload page)
    filterForm.addEventListener('submit', async function(e) {
        ...
        applyBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>Đang lọc...';

        // AJAX filter mode - không reload page
        try {
            const response = await fetch(`/api/tournaments?${params.toString()}&size=12`);
            ...
            log.debug('✅ Filter results:', data);

            // Render filtered results
            renderFilteredTournaments(data.content || data);
            
            // Update URL without reload (cho bookmark/share)
            const newUrl = `/tournaments/list?${params.toString()}`;
            ...
            // Fallback: page reload với query params
            window.location.href = `/tournaments/list?${params.toString()}`;
        } finally {
            // Reset button state
            applyBtn.classList.remove('loading');
            applyBtn.disabled = false;
            applyBtn.innerHTML = '<i class="bi bi-check-circle"></i> Áp dụng bộ lọc';
        }
    });

    /**
     * Render filtered tournaments without page reload
     * @param {Array} tournaments - Array of tournament objects
     */
    function renderFilteredTournaments(tournaments) {
        const container = document.getElementById('tournamentsContainer');
        if (!container) return;
                ...
                container.innerHTML = `
                    <div class="col-12">
                        <div class="empty-state text-center py-5">
                            ...
                            <p class="text-muted">Thử điều chỉnh bộ lọc để xem thêm kết quả</p>
                        </div>
                    </div>
                `;
            } else {
                const currentView = localStorage.getItem('tournament_view_mode') || 'grid';
                
                tournaments.forEach((tournament, index) => {
                    const card = createTournamentCardFromDTO(tournament, currentView);
    ...
    /**
     * Create tournament card from DTO
     * Reusable card creation function
     */
    function createTournamentCardFromDTO(tournament, viewMode) {
        ...
        const status = statusMap[tournament.trangThai] || { text: tournament.trangThai || 'Unknown', class: 'bg-info' };

        // Format date safely
        let startDate = 'TBA';
        if (tournament.ngayBatDau || tournament.ngayBd) {
            try {
                startDate = new Date(tournament.ngayBatDau || tournament.ngayBd).toLocaleDateString('vi-VN');
            } catch (e) {
                startDate = tournament.ngayBatDau || tournament.ngayBd;
            }
        }
    ...
    /**
     * Update stats display after filtering
     */
    function updateFilterStats(data) {
        const totalEl = document.querySelector('.stat-item .stat-value');
        if (totalEl && data.totalElements !== undefined) {
            totalEl.textContent = data.totalElements;
        }
    }
...
function initSortDropdown() {
    ...
    sortOptions.forEach(option => {
        option.addEventListener('click', function(e) {
            ...
            log.debug(`Sort option clicked: ${sortValue}`);
            ...
            if (sortSelectedText) {
                ...
                log.debug(`Updated button text to: ${sortText}`);
            }
            ...
            if (currentSort !== sortValue) {
                ...
                log.debug(`Navigating with sort: ${sortValue}`);
                window.location.href = `/tournaments/list?${params.toString()}`;
            } else {
                log.debug(`Already sorted by: ${sortValue}`);
            }
        });
    });
...
function initInfiniteScroll() {
    log.debug('🔄 Initializing infinite scroll...');
    ...
    async function loadMoreTournaments() {
        if (isLoading || !hasMore) {
            log.debug('⏸️ Skip loading - isLoading:', isLoading, 'hasMore:', hasMore);
            return;
        }
        ...
        try {
            ...
            if (data.tournaments && data.tournaments.length > 0) {
                ...
                log.debug(`Loaded ${data.tournaments.length} tournaments. HasMore: ${hasMore}`);
            } else {
                hasMore = false;
                log.debug('No more tournaments to load');
            }

        } catch (error) {
            log.error('Error loading tournaments:', error);
            showError();
        } finally {
            isLoading = false;
            hideLoading();
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting && hasMore && !isLoading) {
                log.debug('👁️ Scroll trigger detected');
                loadMoreTournaments();
            }
        });
    }, {
...
function initQuickViewModal() {
    ...
    modalEl.addEventListener('shown.bs.modal', function() {
        log.debug('📖 Quick View Modal opened');
    ...
    modalEl.addEventListener('hidden.bs.modal', function() {
        log.debug('📕 Quick View Modal closed');
    ...
    log.debug('✅ Quick View Modal initialized');
}
