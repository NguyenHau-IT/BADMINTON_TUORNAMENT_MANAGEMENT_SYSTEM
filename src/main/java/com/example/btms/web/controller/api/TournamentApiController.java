...
import com.example.btms.service.tournament.TournamentRegistrationService;
...
public class TournamentApiController {
     ...
     * 
     * Request Body: TournamentRegistrationRequest (JSON)
     * 
     * Response: TournamentRegistrationResponse
     * {
     *   "success": true,
    ...
    public ResponseEntity<TournamentRegistrationResponse> registerForTournament(
        ...
        logger.info("POST /api/tournaments/{}/register - email: {}", id, request != null ? request.getEmail() : "null");
        ...
        try {
            ...
            if (tournament == null) {
                ...
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(TournamentRegistrationResponse.error("Giải đấu không tồn tại"));
            }
            
            logger.debug("Tournament found: {} - Status: {}", tournament.getTenGiai(), tournament.getTrangThai());
            
            // Set metadata
            if (request != null) {
                request.setTournamentId(id);
                request.setIpAddress(xForwardedFor != null ? xForwardedFor.split(",")[0] : "unknown");
                request.setUserAgent(userAgent);
            }
            
            // Check if registration is open based on status
            String status = tournament.getTrangThai();
            if (!"registration".equals(status)) {
                logger.warn("Registration not open - Status: {}", status);
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(TournamentRegistrationResponse.error(
                        "Giải đấu không mở đăng ký. Trạng thái hiện tại: " + 
                        getStatusText(status)));
            }
            
            // Additional check with isRegistrationOpen method (includes date validation)
            boolean isRegistrationOpen = tournamentDataService.isRegistrationOpen(id);
            if (!isRegistrationOpen) {
                logger.warn("Registration closed - dates or status check failed");
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(TournamentRegistrationResponse.error("Giải đấu đã đóng đăng ký hoặc chưa mở đăng ký"));
            }
            ...
            if (request == null) {
                ...
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(TournamentRegistrationResponse.error("Dữ liệu đăng ký không hợp lệ"));
            }
            
            // Process registration - Save to database
            TournamentRegistration savedRegistration;
            try {
                savedRegistration = tournamentRegistrationService.registerUser(request);
            } catch (IllegalArgumentException | IllegalStateException e) {
                logger.warn("❌ Validation failed: {}", e.getMessage());
                return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(TournamentRegistrationResponse.error(e.getMessage()));
            }
            
            String registrationCode = savedRegistration.getRegistrationCode();
            String userEmail = savedRegistration.getEmail();
            
            logger.info("✅ Registration successful - ID: {}, code: {}, email: {}", 
                       savedRegistration.getId(), registrationCode, userEmail);
            
            // Check if payment is required
            if (tournament.getPhiThamGia() != null && tournament.getPhiThamGia().doubleValue() > 0) {
                String paymentInstructions = String.format(
                    "Vui lòng chuyển khoản %s VNĐ đến:\n" +
                    "Ngân hàng: Vietcombank\n" +
                    "STK: 1234567890\n" +
                    "Nội dung: %s",
                    java.text.NumberFormat.getInstance().format(tournament.getPhiThamGia()),
                    registrationCode
                );
                
                return ResponseEntity.status(HttpStatus.CREATED)
                    .body(TournamentRegistrationResponse.successWithPayment(
                        id,
                        registrationCode,
                        userEmail,
                        paymentInstructions
                ...
                return ResponseEntity.status(HttpStatus.CREATED)
                    .body(TournamentRegistrationResponse.success(
                        id,
                        registrationCode,
                        userEmail
                    ));
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(TournamentRegistrationResponse.error("Đăng ký thất bại: " + e.getMessage()));
        }
    }
     ...
     * - email: User's email
     * 
     * Response: { "registered": true/false, "registrationCode": "REG-XXX" }
     */
    ...
    public ResponseEntity<Map<String, Object>> checkRegistration(
        ...
        logger.debug("GET /api/tournaments/{}/registration/check - email: {}", id, email);
        ...
        try {
            ...
            response.put("registrationCode", null);
            
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            logger.error("Check registration failed: {}", e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
