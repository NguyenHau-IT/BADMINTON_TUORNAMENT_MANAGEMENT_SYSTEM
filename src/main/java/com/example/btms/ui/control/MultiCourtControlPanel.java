...
public class MultiCourtControlPanel extends JPanel implements PropertyChangeListener {
    ...
    // Mapping giữa tên hiển thị sân và UUID v7 thực tế
    private final Map<String, String> courtDisplayToUuidMap = new HashMap<>();
    ...
    private JPanel createHeaderPanel() {
        ...
        // Combo box chọn sân
        // Tạo danh sách hiển thị "Sân 1" đến "Sân 10" với UUID v7 mapping
        JComboBox<String> courtCombo = new JComboBox<>();

        // Tạo mapping cho 10 sân đầu tiên
        initializeCourtMapping(courtCombo);
    ...
    private void ensureTabsForExistingCourts() {
        try {
            ...
            for (String courtId : all.keySet()) {
                ...
                String expectedTitle = formatCourtIdForDisplay(courtId);
                for (int i = 0; i < courtTabs.getTabCount(); i++) {
                    if (expectedTitle.equals(courtTabs.getTitleAt(i))) {
                        exists = true;
                        break;
                    }
                }
    ...
    private void refreshOverview() {
        ...
        if (allStatus.isEmpty()) {
            ...
            allStatus.values().stream()
                    .sorted((s1, s2) -> {
                        // Sắp xếp theo hashCode của UUID để có thứ tự nhất quán
    ...
    /**
     * Trích xuất thứ tự từ courtId để sắp xếp
     * Với UUID v7, sắp xếp theo timestamp embedded
     */
    private int extractCourtNumber(String courtId) {
        if (courtId == null)
            return 0;

        // Với UUID v7, có thể sắp xếp theo string (time-ordered)
        // hoặc sử dụng hashCode để có thứ tự nhất quán
        return Math.abs(courtId.hashCode()) % 1000;
    }

    /**
     * Hiển thị tên sân cho user interface
     */
    private String formatCourtIdForDisplay(String courtId) {
        if (courtId == null) {
            return courtId;
        }

        // Tìm display name từ mapping (UUID -> display name)
        for (Map.Entry<String, String> entry : courtDisplayToUuidMap.entrySet()) {
            if (entry.getValue().equals(courtId)) {
                return entry.getKey(); // Trả về "Sân 1", "Sân 2", etc.
            }
        }

        // Fallback: hiển thị UUID ngắn nếu không tìm thấy mapping
        if (courtId.length() < 8) {
            return courtId;
        }
        return "Sân " + courtId.substring(0, 8) + "...";
    }
    ...
    private JPanel createCourtStatusCard(CourtStatus status) {
        ...
        JLabel headerLabel = new JLabel(formatCourtIdForDisplay(status.courtId) + " - " + status.header);
    ...
    private void addNewCourtFromCombo(JComboBox<String> courtCombo) throws SQLException {
        ...
        if (displayName == null || courtId == null) {
    ...
    /**
     * Lấy UUID từ display name
     */
    private String getCourtUuidFromDisplayName(String displayName) {
        return courtDisplayToUuidMap.get(displayName);
    }
    ...
    private void selectNextAvailableCourt(JComboBox<String> courtCombo) {
        // Tìm sân tiếp theo chưa được tạo
        for (int i = 0; i < courtCombo.getItemCount(); i++) {
            String displayName = courtCombo.getItemAt(i);
            String uuid = getCourtUuidFromDisplayName(displayName);
            if (uuid != null && courtManager.getCourt(uuid) == null) {
                courtCombo.setSelectedItem(displayName);
                return;
            }
        }

        // Nếu tất cả sân đều đã được tạo, chọn sân đầu tiên
        if (courtCombo.getItemCount() > 0) {
