...
public class DoiRepository {
    ...
    public List<DangKiDoi> fetchTeamsByNoiDungVaGiai(int idNoiDung, int idGiai) {
        ...
        // SQL query tương thích với cả SQL Server và H2
        final String sql = "SELECT DISTINCT " +
                "  COALESCE(dkd.TEN_TEAM, CAST(dkd.ID_TEAM AS VARCHAR(50))) AS LBL, " +
                "  dkd.ID_TEAM " +
                "FROM DANG_KI_DOI dkd " +
    ...
    public VanDongVien[] fetchTeamPlayersDetailed(int teamId) {
        ...
        // Lấy tối đa 2 dòng - syntax tương thích với H2
        final String sql = "SELECT " +
                "  ctd.ID_VDV, " +
                "  COALESCE(vdv.HO_TEN, CAST(ctd.ID_VDV AS VARCHAR(50))) AS NM " +
                "FROM CHI_TIET_DOI ctd " +
                "LEFT JOIN VAN_DONG_VIEN vdv ON vdv.ID = ctd.ID_VDV " +
                "WHERE ctd.ID_TEAM = ? " +
                "ORDER BY ctd.ID_VDV LIMIT 2";
    ...
    public String fetchClubNameById(int teamId) {
        ...
        final String sql = "SELECT clb.TEN_CLB FROM DANG_KI_DOI dkd " +
                "LEFT JOIN CAU_LAC_BO clb ON clb.ID = dkd.ID_CLB " +
                "WHERE dkd.ID_TEAM = ?";
